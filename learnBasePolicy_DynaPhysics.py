...
max_timesteps = 11e5 # Total number of iterations/timesteps
...
if logginWB:
   import wandb
   wandb.init(project="dmWalker__dynaPhy_train",name=f"TrainBasepolicy",save_code=True)
  ...
  # If the episode is done
    if done:
        ...
        if timesteps_since_eval >= eval_freq:
            ...
            policy.save(file_name+str(itSave), directory="./pytorch_models")
    ...
    qPos = np.array(env.physics.data.qpos.copy(), dtype=np.float32)
    qVel = np.array(env.physics.data.qvel.copy(), dtype=np.float32)
    new_obs = np.concatenate((qPos, qVel))
...
if logginWB:
    wandb.init(project="dmWalker__dynaPhy_train",name=f"evaluation_basepolicy",save_code=True)
